...
{
    /**
     * Étudiants autorisés à être sélectionnés :
     * seulement les membres qui habitent dans un logement
     * dont nbrDeplace est supérieur à 15.
     */
    private function eligibleStudentsQuery()
    {
        return User::role('Membres')
            ->with(['roles', 'logement'])
            ->whereHas('logement', function ($query) {
                $query->where('nbrplace', '>', 15);
            });
    }
    ...
    public function create()
    {
        // Ici on affiche seulement les étudiants qui habitent dans une salle d'étude
        $users = $this->eligibleStudentsQuery()
            ->orderBy('name')
            ->get();

        // Les logements cibles restent tous disponibles
        $logements = Logement::orderBy('name')->get();
    ...
    public function store(Request $request)
    {
        $eligibleUserIds = $this->eligibleStudentsQuery()
            ->pluck('users.id')
            ->map(fn ($id) => (int) $id)
            ->toArray();
        ...
        $request->validate([
            ...
            'attributions.*.logement_id' => [
                'required',
                'integer',
                'exists:logements,id',
            ],

            'attributions.*.user_ids' => [
                'required',
                'array',
                'min:1',
            ],
            ...
            'attributions.*.user_ids.*' => [
                'required',
                'integer',
                Rule::in($eligibleUserIds),
            ],

            'attributions.*.date_debut' => [
                'required',
                'date',
            ],

            'attributions.*.date_fin' => [
                'nullable',
                'date',
                'after_or_equal:attributions.*.date_debut',
            ],
        ], [
            'attributions.*.user_ids.*.in' => 'Vous ne pouvez attribuer que les étudiants qui habitent dans une salle d’étude.',
        ]);
    ...
    {
        $users = $this->eligibleStudentsQuery()
            ->orWhere('users.id', $attribution->user_id)
            ->orderBy('name')
            ->get();
        ...
        return Inertia::render('Attributions/Edit', [
            'attribution' => $attribution->load(['user.roles', 'logement']),
            'users' => $users,
            'logements' => $logements,
        ]);
    }
    ...
    public function update(Request $request, Attribution $attribution)
    {
        ...
        $request->validate([
            ...
            'user_id.in' => 'Vous ne pouvez attribuer que les étudiants qui habitent dans une salle d’étude.',
        ]);

        $attribution->update([
            'user_id' => (int) $request->user_id,
            'logement_id' => (int) $request->logement_id,
        ...
        return redirect()
            ->route('attributions.index')
            ->with('success', 'Attribution mise à jour.');
    }
